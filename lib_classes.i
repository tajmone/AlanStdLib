...
-- >>> devworn >>> TODO: delete when everything is working wihtout 'worn'.
THE worn ISA ENTITY
  CONTAINER TAKING CLOTHING.
    HEADER SAY my_game:hero_worn_header.
...
ADD TO EVERY definition_block
  ...
  HAS temp_blocking_worn { clothing }. --> Temporary Set used by the library
                                       --  to track clothes blocking wear/remove
                                       --  and listing them in response.(ignore)
...
EVERY clothing ISA OBJECT
  ...
  INITIALIZE
    ...
    -- NOTE: | The set attribute 'IS wearing' is defined to work for both
    --       | the Hero as well as for NPCs.

    FOR EACH ac ISA ACTOR
      DO
        -- >>> devworn >>> tweaked:
        -- * removed handling of Hero via worn.
        -- * use 'donned' as criteria for items being worn.
        ------------------------------------------------------------
        -- Any clothing item directly inside an actor and marked as
        -- 'donned' must also be added to the actor's 'wearing' set.
        ------------------------------------------------------------
...
--------------------------------------------------------------------
      ...
      SET tempcovered OF hero TO
        SUM OF topcover IsA CLOTHING, DIRECTLY IN hero, IS donned /2.
        ...
        THEN
          ...
          FOR EACH blocking_item IsA CLOTHING, DIRECTLY IN hero, IS donned
            ...
            DO DEPENDING ON botcover OF blocking_item
              = 64 --> coat-like
                THEN "" -- Do nothing.
                -- Coats don't block removing lower-body-only garments.
...
--------------------------------------------------------------------
      ...
      -- <<< original code <<<
          ...
          -- >>> devworn >>> tweaked:
          "In order to remove $+1 you should first remove"
          -- -----------------
          -- List blocking clothes
          -- -----------------
          SET my_game:temp_cnt TO COUNT IsA CLOTHING, IN temp_blocking_worn OF my_game.
        ...
        -- The original tweak (below) listed every worn item, like before.
        -- Currently being replaced by the new code (above), which only lists
        -- those worn clothes which are preventing removing the desired item.
        -- 
...
EVENT worn_clothing_check
  FOR EACH ac ISA ACTOR
    DO
      -- >>> devworn >>> TODO:
      -- * Handle clothing indirectly IN actor: they should be carried!
      -- >>> devworn >>> tweaked:
      -- * Removed handling of Hero via worn.
      -- * Use 'donned' as check criteria, instead of 'wearing'.
      ------------------------------------------------------------
      -- Any clothing item directly inside an actor and marked as
      -- 'donned' must also be added to the actor's 'wearing' set.
      ------------------------------------------------------------
